...
lines += ['', f"Latest observations: {len(latest):,}", f"Average BEHS: {latest.behs.mean():.2f}"]
...
root=q("select issue_category, count(*) as observations from sentiment_scores where issue_category <> 'NO_ISSUE' group by issue_category order by observations desc")
lines += ['', '## Feedback issue taxonomy (concern comments only)', root.to_markdown(index=False)]
